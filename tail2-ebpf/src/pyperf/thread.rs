...
pub fn get_thread_state<C: BpfContext>(ctx: &C, state: &mut SampleState) -> Result<usize, ErrorCode> {
    for i in 0..THREAD_STATES_PROG_CNT {
        ...
        if i == THREAD_STATES_PROG_CNT {
            return Err(ErrorCode::ERROR_TOO_MANY_THREADS);
        }
    }
...
pub unsafe fn get_task_thread_id<C: BpfContext>(ctx: &C, task: *const task_struct, pthreads_impl: pthreads_impl) -> Result<u64, ErrorCode> {
